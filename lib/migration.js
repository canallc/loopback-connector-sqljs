...
function mixinMigration(SQLiteDB) {
  ...
  function getTableStatus(model, cb) {
    console.log('Inside getTableStatus for model', model);
  ...
  SQLiteDB.prototype.mapToDB = function (model, data) {
    console.log('inside mapToDB for model: ', model);
    var dbData = {};
    if (!data) {
      return dbData;
    }
    ...
    console.log('inside mapToDB for model: ', model, 'About to return: ', dbData);
    return dbData;
  };
